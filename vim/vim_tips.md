...
1. "f" 表示 "Find"。 例如，假定你在下面例子行的行首，而你想移动到单词 "human" 的h那里
2. "F" 命令用于向左查找.
3. "tx" 命令与 "fx" 相似，但它只把光标移动到目标字符的前一个字符上。提示："t" 表示 "To"。这个命令的反向版本是 "Tx"。
4. 这四个命令可以通过 ";" 命令重复，"," 命令则用于反向重复


[advanced commands in Vim: t,f,m,e,J ](http://blog.sina.com.cn/s/blog_605f5b4f0101f0xz.html)
